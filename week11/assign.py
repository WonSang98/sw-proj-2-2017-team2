...
class Calculator(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        functiondic = {'factorial (!)' : factorial, '-> binary': decToBin, 'binary -> dec': binToDec,
    '-> roman': decToRoman}

        constantdic = { 'pi':3.141592, '빛의 이동 속도 (m/s)': 3E+8, '소리의 이동 속도 (m/s)': 340,
    ...
    def buttonClicked(self):
        ...
        # = 눌리면 창에입력된 연산 실행하고 결과창에표시
            ...
            #함수 버튼 입력 했을때
            n = self.display.text()
            self.display.setText(functiondic[key](n))

        elif key in constatntList:
            self.display.setText(self.display.text()+constantdic[key])

        #elif key == 'factorial (!)':
        #   n = self.display.text()
        #   self.display.setText(factorial(n))
            #팩토리얼 계산.
        else:
            if self.display.text() == ''and key in operator:
            #연산기호부터 입력못하도록 예외처리.
                pass
            #위에서 안걸리면 이제 처음이 연산기호가 아닌 수로 시작
            ...
            #연달아 연산기호가 입력되지 않도록..예외처리
            #self.display.text()가 비어있을때 인덱스[-1]하니까 에러남..
            #아..근데 **랑 //는 허용해야하는디..머리아프다.
            #    pass
